...
export const sendResetCode = (email, name, code) => {
    ...
    const mailOptions = {
        from: EMAIL,
        to: email,
        subject: "Reset frogBook password",
        html: `<div style="max-width:700px;margin-bottom:1rem;display:flex;align-items:center;gap:10px;font-family:Roboto;font-weight:600;color:#3b5998"><img src="https://www.dropbox.com/s/vo2mqu6jx19dog6/Resized_svg.svg?raw=1"
alt="" style="width:30px"><span>Action request : Activate your frogBook account</span></div>
<div style="padding:1rem 0;border-top:1px solid #e5e5e5;border-bottom:1px solid #e5e5e5;color:#141823;font-size:17px;font-family:Roboto"><span>Hello ${name}</span>
<div style="padding:20px 0"><span style="padding:1.5rem 0">You recently created an account on frogBook. To complete your registration, please confirm your account.</span></div>
<a  style="width:200px;padding:10px 15px;background:#4c649b;color:#fff;text-decoration:none;font-weight:600">${code}</a><br>
<div style="padding-top:20px"><span style="margin:1.5rem 0;color:#898f9c">frogBook allows you to stay in touch with all your friends, once registered on frogBook, you can share photos,organize events and much more.</span>
</div></div>`,
    }
    smtp.sendMail(mailOptions, (err, res) => {
        if (err) return err
